...
class AttendanceController extends Controller
{
    ...
    public function showDeletedData(Request $request)
    {
        ...
        $data['funinit'] = array(
            'Attendance.trash_init()',
        );
        $data['header'] = array(
            'title' => 'Attendance Deleted List',
            'breadcrumb' => array(
                'Dashboard' => route('my-dashboard'),
                'Attendance Deleted List' => 'Attendance Deleted List',
            )
        );
        return view('backend.pages.attendance.trash', $data);
    }
    ...
    public function ajaxcall(Request $request)
    {
        ...
        switch ($action) {
            ...
            case 'get-attendance-data':
                $objAttendance = new Attendance();
                $list = $objAttendance->getAttendanceDatatable($request->input('data'));
                echo json_encode($list);
                break;
            ...
            case 'common-activity':
                ...
                if ($result) {
                    ...
                    if ($data['activity'] == 'restore-records') {
                        $attendanceId = $data['id'];
                        $attendanceRecord = Attendance::find($attendanceId);
                        $date = date_formate($attendanceRecord['date']);
                        $return['message'] = "Attendance details successfully restore.";
                    }
