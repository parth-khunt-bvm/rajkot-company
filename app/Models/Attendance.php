...
class Attendance extends Model
{
    ...
    //         // 3 => 'attendance.attendance_type',
    //         3 => DB::raw('(CASE WHEN attendance.attendance_type = "o" THEN "Actived" ELSE "Deactived" END)'),
    //         4 => 'attendance.reason',
    //     );
    ...
    //             $attendance_type = '<span class="label label-lg label-light-success label-inline">Active</span>';
    ...
    //         } else {
    //             $attendance_type = '<span class="label label-lg label-light-danger  label-inline">Deactive</span>';
    ...
    public function saveAdd($requestData)
    {
        ...
        $uniqueValue = array_diff($employee, $employeeId);
    ...
    public function get_admin_attendance_details(){
        ...
        //     ->count();
            ...
            $sort_leave_array =[];
                    ...
                    ->count();
                    // array_push($present_array, $present);

                    $absent = Attendance::from('attendance')
                    ->where('attendance_type', '1')
                    ->count();
                    // array_push($absent_array, $absent);

                    $half_day = Attendance::from('attendance')
                    ->where('attendance_type', '2')
                    ->count();
                    // array_push($half_day_array, $half_day);

                    $sort_leave = Attendance::from('attendance')
                    ->where('attendance_type', '3')
                    ->count();
                    // array_push($sort_leave_array, $sort_leave);

            $details['amount'] = ['present' => $present, 'absent' => $absent, 'half_day' => $half_day, 'sort_leave' => $sort_leave];
            return $details;
    }
}
